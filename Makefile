...
bin_PROGRAMS = http_client$(EXEEXT) http_server$(EXEEXT)
subdir = .
...
am_http_client_OBJECTS = client.$(OBJEXT)
http_client_OBJECTS = $(am_http_client_OBJECTS)
http_client_DEPENDENCIES =
am_http_server_OBJECTS = server.$(OBJEXT) server_receive.$(OBJEXT) \
	http_fetcher.$(OBJEXT)
...
CXXLINK = $(LIBTOOL) --tag=CXX $(AM_LIBTOOLFLAGS) $(LIBTOOLFLAGS) \
	...
	$(LDFLAGS) -o $@
SOURCES = $(http_client_SOURCES) $(http_server_SOURCES)
DIST_SOURCES = $(http_client_SOURCES) $(http_server_SOURCES)
ETAGS = etags
CTAGS = ctags
...
ACLOCAL = ${SHELL} /search/wuwenjun/project/http_select/_aux/missing --run aclocal-1.11
AMTAR = ${SHELL} /search/wuwenjun/project/http_select/_aux/missing --run tar
AR = ar
AUTOCONF = ${SHELL} /search/wuwenjun/project/http_select/_aux/missing --run autoconf
AUTOHEADER = ${SHELL} /search/wuwenjun/project/http_select/_aux/missing --run autoheader
AUTOMAKE = ${SHELL} /search/wuwenjun/project/http_select/_aux/missing --run automake-1.11
AWK = gawk
BUILD_DEPENDENCIES = /search/wuwenjun/project/http_select
CC = gcc -std=gnu99
...
LN_S = ln -s
LTLIBOBJS = 
MAKEINFO = ${SHELL} /search/wuwenjun/project/http_select/_aux/missing --run makeinfo
MKDIR_P = /bin/mkdir -p
NM = /usr/bin/nm -B
NMEDIT = 
OBJDUMP = objdump
...
STRIP = strip
VERSION = 2.0.0
abs_builddir = /search/wuwenjun/project/http_select
abs_srcdir = /search/wuwenjun/project/http_select
abs_top_builddir = /search/wuwenjun/project/http_select
abs_top_srcdir = /search/wuwenjun/project/http_select
...
install_sh = ${SHELL} /search/wuwenjun/project/http_select/_aux/install-sh
...
http_client$(EXEEXT): $(http_client_OBJECTS) $(http_client_DEPENDENCIES) 
	...
	$(CXXLINK) $(http_client_OBJECTS) $(http_client_LDADD) $(LIBS)
http_server$(EXEEXT): $(http_server_OBJECTS) $(http_server_DEPENDENCIES) 
	@rm -f http_server$(EXEEXT)
	$(CXXLINK) $(http_server_OBJECTS) $(http_server_LDADD) $(LIBS)
...
include ./$(DEPDIR)/http_fetcher.Po
include ./$(DEPDIR)/server.Po
include ./$(DEPDIR)/server_receive.Po
...
TAGS:  $(HEADERS) $(SOURCES) package_config.h.in $(TAGS_DEPENDENCIES) \
		$(TAGS_FILES) $(LISP)
	set x; \
	here=`pwd`; \
...
ctags: CTAGS
CTAGS:  $(HEADERS) $(SOURCES) package_config.h.in $(TAGS_DEPENDENCIES) \
		$(TAGS_FILES) $(LISP)
	list='$(SOURCES) $(HEADERS) package_config.h.in $(LISP) $(TAGS_FILES)'; \
	unique=`for i in $$list; do \
	    if test -f "$$i"; then echo $$i; else echo $(srcdir)/$$i; fi; \
...
all-am: Makefile $(PROGRAMS) package_config.h all-local
installdirs:
